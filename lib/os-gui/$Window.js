	...
	function $Window(options) {
		...
		$w.focus = () => {
			if (options.parentWindow) {
				...
				setTimeout(() => {
					// wait til after blur handler of parent window
		...
		$G.on("pointerdown", (e) => {
			if (e.target.closest(".os-window") !== $w[0] && !e.target.closest(".taskbar")) {
				$w.blur();
			}
		});
		...
		$w.on("keydown", (e) => {
			...
			switch (e.keyCode) {
				...
				case 9: {
					...
					// @#: focusables
					let $controls = $w.$content
						.find("input, textarea, select, button, object, a[href], [tabIndex='0'], details summary")
		...
		$w.bringTitleBarInBounds = () => {
			...
			$w.css({
				left: Math.max(
					...
					Math.min(document.body.scrollWidth - min_horizontal_pixels_on_screen, $w.position().left)
				),
				top: Math.max(0, Math.min(document.body.scrollHeight - $w.$titlebar.outerHeight() - 5, $w.position().top)),
			});
		};
		...
		$G.on("pointerup pointercancel", (e) => {
			if (e.pointerId !== drag_pointer_id) {
				return;
			}
		...
		if (options.resizable) {
			...
			[
				...
				function handle_pointermove(e) {
					if (e.pointerId !== resize_pointer_id) {
						return;
					}
					resize_pointer_x = e.clientX;
					resize_pointer_y = e.clientY;

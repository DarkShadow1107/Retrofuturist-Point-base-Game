		...
		function $MenuPopup(menu_items) {
			...
			$.map(menu_items, (item) => {
				...
				if (item === MENU_DIVIDER) {
					...
					if (item.submenu) {
						$submenu_area.html(
							'<svg xmlns="http://www.w3.org/2000/svg" width="10" height="11" viewBox="0 0 10 11" style="fill:currentColor;display:inline-block;vertical-align:middle"><path d="M7.5 4.33L0 8.66L0 0z"/></svg>'
						);
						...
						const open_submenu = () => {
							...
							$submenu_popup.css({
								...
								left:
									(get_direction() === "rtl" ? rect.left - submenu_popup_rect.width : rect.right) +
									window.scrollX,
		...
		const make_menu = (menus_key, menu_items) => {
			...
			$G.on("keydown", (e) => {
				if (e.ctrlKey || e.metaKey) {
					// Ctrl or Command held
					if (e.keyCode !== 17 && e.keyCode !== 91 && e.keyCode !== 93 && e.keyCode !== 224) {
						// anything but Ctrl or Command pressed
						close_menus();
					}
					return;
				}
